...
// //   const { examid } = req.params;
// //   try {
// //     // Fetch instructions from the database based on examid
// //     const [instructionsRows] = await db.query(
// //       "SELECT t1.instructionHeading, t2.points, cct.examid, tct.courseCreationId FROM instruction t1 JOIN instructions_points t2 ON t1.instructionid = t2.instructionid JOIN course_creation_table cct ON t1.examid = cct.examid JOIN test_creation_table tct ON cct.courseCreationId = tct.courseCreationId WHERE t1.examid = ?",
// //       [examid]
// //     );
// //     res.json(instructionsRows);
// //   } catch (error) {
...
// //     // Fetch instructions from the database based on examid
// //     const [instructionsRows] = await db.query(
// //       "SELECT DISTINCT t1.instructionHeading, t2.points, cct.examid, tct.courseCreationId FROM instruction t1 JOIN instructions_points t2 ON t1.instructionid = t2.instructionid JOIN course_creation_table cct ON t1.examid = cct.examid JOIN test_creation_table tct ON cct.courseCreationId = tct.courseCreationId WHERE t1.examid = 8;",
// //       [examid]
// //     );
// //     res.json(instructionsRows);
// //   } catch (error) {
...
// //     // Fetch instructions from the database based on testCreationTableId
// //     const [instructionsRows] = await db.query(
// //       "SELECT t1.instructionHeading, t2.points, cct.examid, tct.courseCreationId FROM instruction t1 JOIN instructions_points t2 ON t1.instructionid = t2.instructionid JOIN course_creation_table cct ON t1.examid = cct.examid JOIN test_creation_table tct ON cct.courseCreationId = tct.courseCreationId WHERE t1.examid = 8;", [examid]
// //     );
// //     res.json(instructionsRows);
// //   } catch (error) {
// //     console.error(error);
...
// // SELECT t1.instructionHeading, t2.points, cct.examid, tct.courseCreationId
// // FROM instruction t1
// // JOIN instructions_points t2 ON t1.instructionid = t2.instructionid
// // JOIN course_creation_table cct ON t1.examid = cct.examid
// // JOIN test_creation_table tct ON cct.courseCreationId = tct.courseCreationId
// // WHERE t1.examid = 8;
...
//     const [subjects] = await db.query(
//       'SELECT subjects.subjectName,subjects.subjectId FROM test_creation_table JOIN course_creation_table ON test_creation_table.courseCreationId = course_creation_table.courseCreationId JOIN course_subjects ON course_creation_table.courseCreationId = course_subjects.courseCreationId JOIN Subjects ON course_subjects.subjectId = Subjects.subjectId WHERE test_creation_table.testCreationTableId = 8',
//       [testCreationTableId]
//     );
//     res.json(subjects);
//   } catch (error) {
...
// async function calculateQuestionLength(sectionId) {
//   try {
//     // Replace 'questions' with your actual table name
//     const [result] = await db.query(
//       "SELECT COUNT(*) AS questionLength FROM questions WHERE sectionId = ?",
//       [sectionId]
//     );
// console.log(sectionId)
// // console.log(questionLength)
//     // Extract the questionLength from the database result
//     const questionLength = result[0] ? result[0].questionLength : 0;

//     return questionLength;
//   } catch (error) {
//     console.error("Error calculating question length:", error);
...
//     const sectionsWithQuestionLength = await Promise.all(rows.map(async (section) => {
//       const questionLength = await calculateQuestionLength(section.sectionId);
//       return { ...section, questionLength };
//     }));

//     console.log(sectionsWithQuestionLength)
...
//     const sectionsWithTotalQuestions = rows.map((section) => ({
//       ...section,
//       totalQuestions: section.noOfQuestions.reduce((acc, curr) => acc + curr, 0),
//     }));

//     res.json(sectionsWithTotalQuestions);
//   } catch (error) {
//     console.error(error);
//     res.status(500).json({ error: "Internal Server Error" });
//   }
...
// app.get("/fetchSections/:sectionId", async (req, res) => {
//   const { sectionId } = req.params;
//   try {
//     const [rows] = await db.query(
//       "SELECT sectionId FROM sections WHERE sectionId=?",
//       [sectionId]
//     );
//     res.json(rows);
//   } catch (error) {
...
// app.get("/getPaperData/:testCreationTableId/:subjectId/:sectionId", async (req, res) => {
//   try {
//     const subjectId = req.params.subjectId;
//     const testCreationTableId = req.params.testCreationTableId;
//     const sectionId = req.params.sectionId;

//     console.log("Parameters:", testCreationTableId, subjectId, sectionId);

 
//     // Fetch data from testCreationTableId table
//     const testData = await getDataByTestCreationTableId(testCreationTableId);
 
//     // Fetch question data based on subjectId, sectionId, and document_Id
//     const questions = await getQuestionsBySubjectAndDocumentId(subjectId, sectionId, testCreationTableId);
 
//     // Fetch option data based on questions, sectionId, and document_Id
//     const options = await getOptionsByQuestionsAndDocumentId(questions, sectionId, testCreationTableId);
 
//     // Fetch solution data based on questions, sectionId, and document_Id
//     const solutions = await getSolutionsByQuestionsAndDocumentId(questions, sectionId, testCreationTableId);
...
// // Modify the functions to include sectionId
// async function getQuestionsBySubjectAndDocumentId(subjectId, testCreationTableId,sectionId) {
//   try {
//     const query = `
//       SELECT question_id, question_img
//       FROM questions
...
//     `;
//     const [results] = await db.query(query, [subjectId, testCreationTableId,sectionId]);
//     const optionsWithBase64 = results.map(option => ({
//       question_id: option.question_id,
//       question_img: option.question_img.toString('base64'),
//     }));
...
//       // If there are no questionIds, return an empty array or handle as needed
//       return [];
//     }
...
//     // Fetch question data based on subjectId, sectionId, and document_Id
//     const questions = await getQuestionsBySubjectAndDocumentId(subjectId, testCreationTableId, sectionId);
...
// // Reusable function to get questions data based on subjectId, sectionId, and document_Id
// async function getQuestionsBySubjectAndDocumentId(subjectId, testCreationTableId, sectionId) {
//   try {
//     const query = `
//       SELECT question_id, question_img
